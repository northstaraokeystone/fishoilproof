...
The "Kill" Moment: When the system blocks an attack (e.g., "WRITE REJECTED"), the UI must:
...
The Output: Every demo MUST culminate in a generated artifact that looks court-admissible:
...
The Money Shot: The artifact must contain:
...
The Attack: Explicitly call out the "Lazy Standard":
...
The Pivot: Move conversation from:
...
The Kill Shot: Always include comparison slide that disqualifies competitor on architectural grounds:
...
Alive State: Even when idle, system must show faint "System Armed" indicator:
